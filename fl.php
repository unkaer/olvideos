...
function getlist($api,$t,$f){
    $data = file_get_contents($api."?ac=list&t=".$t);
    $xml = simplexml_load_string($data);
...
function build($f){
    global $array,$n,$file,$name,$fw,$cid;
    ...
    if(isset($array[$n]["title"])){
        if( (!strstr($array[$n]["type"],"伦理")) && (!strstr($array[$n]["type"],"福利")) ){   // 屏蔽部分搜索结果
            print_r('<div id="playul"><p>'.$array[$n]["zy"].'</p><div><a id="cover" href="./play.php?wd=api'.$fw.'id'.$cid.'&f=1&id='.$n.'" target="_blank" title="'.$array[$n]["des"].'" style="background-image: url('.$array[$n]["cover"].')">');  // 封面
